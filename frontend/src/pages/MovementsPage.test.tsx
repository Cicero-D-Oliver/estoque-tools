...
describe('histórico operacional de movimentações', () => {
  ...
  it('destaca para ADMIN o que aguarda confirmação', async () => {
    const { fetchMock } = await openMovements()
    expect(screen.getByText('1 movimentação aguardando confirmação do admin.')).toBeInTheDocument()
    expect(screen.getByText('Aguardando confirmação do admin.')).toBeInTheDocument()
  ...
  it('confirma administrativamente sem enviar payload operacional e atualiza o histórico', async () => {
    ...
    expect(dialog).toHaveTextContent('Furadeira Bosch')
    expect(dialog).toHaveTextContent('Retirada')
    expect(dialog).not.toHaveTextContent('A operação já foi realizada')
    expect(dialog).not.toHaveTextContent('conferência administrativa')
    expect(within(dialog).getByRole('button', { name: 'Não' })).toBeInTheDocument()
    await userEvent.click(within(dialog).getByRole('button', { name: 'Sim' }))
    ...
    await waitFor(() => expect(screen.getByText('Sem novas movimentações')).toBeInTheDocument())
    expect(screen.queryByText('Nada aguardando conferência.')).not.toBeInTheDocument()
  })
  ...
  it('preserva no mobile quando, item, executor, resultado e conferência sem overflow', async () => {
    ...
    expect(row).toHaveTextContent('Aguardando confirmação do admin.')
